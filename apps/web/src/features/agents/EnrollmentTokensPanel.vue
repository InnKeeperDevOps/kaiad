...
function buildKaiadAgentManifest(opts: {
  ...
  const lines = [
    ...
    "  manages:",
    // Cluster-wide (no namespaceSelector) so the operator generates a
    // ClusterRole/ClusterRoleBinding and the agent can deploy a
    // monitored service into WHATEVER namespace its kaiad.yaml resolves
    // to — self-provisioning that namespace (namespaces create) and its
    // own image-pull Secret (secrets create, write-only) — with zero
    // per-namespace admin setup. deployments/services/ingresses get the
    // full create/update/delete the redeploy/teardown handlers need.
    "    - apiGroups: [\"apps\"]",
